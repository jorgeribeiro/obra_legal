	...
	public void carregarObrasPorMandato() {
		try {
			obras = obraServicoFacade.findObrasServicosByTipoExecucaoAndTipoObraServicoAndMandato(
					TipoExecucao.EXECUCAO_INDIRETA, TipoObraServico.OBRA,
					usuarioObraLegal.getMandato2().getMandatoId());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void abrirModalPreencheDadosObra(@BindingParam("visible") boolean visible) {
		try {
			if (visible) {
				...
				obraServico.setTipoExecucao(TipoExecucao.EXECUCAO_INDIRETA);
